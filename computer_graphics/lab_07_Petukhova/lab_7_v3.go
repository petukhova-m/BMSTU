...
func calcCylinder(rad_a float32, rad_b float32, c_height float32, step1 int, step2 int) *Cylinder {
	...
	for angle := 0.0; angle < 2*Pi; angle += e_step {
		...
		p1t := []Point{{coords: t2t, n: []float32{0.0, 1.0, 0.0}, tCoords: tx01},
			...
			{coords: t1t, n: []float32{0.0, 1.0, 0.0}, tCoords: []float32{0.0, 0.0}}}
		...
		p1b := []Point{{coords: t3b, n: []float32{0.0, -1.0, 0.0}, tCoords: tx11},
			...
			{coords: t1b, n: []float32{0.0, -1.0, 0.0}, tCoords: []float32{0.0, 0.0}}}
	...
	return &Cylinder{a: rad_a, b: rad_b, h: c_height, e_step: step1, h_step: step2, points: points, count: count}
}
...
func drawCylinder()  {
	...
	gl.InterleavedArrays(gl.T2F_N3F_V3F, 0, gl.Ptr(&cyl.points[0]))
