...
func calcCylinder(rad_a float64, rad_b float64, c_height float64, step1 int, step2 int) *Cylinder {
	...
	var pointsT, pointsB []Point
	var pointsSide [][]Point
	pointsT = append(pointsT, Point{coords: t1t, n: []float32{0.0, 1.0, 0.0}, tCoords: []float32{0.0, 0.0}, aCoords: t1t})
	pointsB = append(pointsB, Point{coords: t1b, n: []float32{0.0, -1.0, 0.0}, tCoords: []float32{0.0, 0.0}, aCoords: t1b})
	...
	for angle := 0.0; angle < 2*Pi; angle += e_step {
		...
		p1t := Point{coords: t5t, n: []float32{0.0, 1.0, 0.0}, tCoords: tx10, aCoords: t1Anim}
		p1b := Point{coords: t5b, n: []float32{0.0, -1.0, 0.0}, tCoords: tx10, aCoords: t1Anim}
		pointsT = append(pointsT, p1t)
		pointsB = append(pointsB, p1b)
		pointsT = append(pointsT, Point{coords: t1t, n: []float32{0.0, 1.0, 0.0}, tCoords: []float32{0.0, 0.0}, aCoords: t1t})
		pointsB = append(pointsB, Point{coords: t1b, n: []float32{0.0, -1.0, 0.0}, tCoords: []float32{0.0, 0.0}, aCoords: t1b})

		pointsS := make([]Point, 0)
		for i := 0; i <= step2; i++ {
			...
			p1s := Point{coords: []float64{x1, y, z1}, n: *normalize(x1, 0, z1), tCoords: t10, aCoords: t1Anim}
			p2s := Point{coords: []float64{x2, y, z2}, n: *normalize(x2, 0, z2), tCoords: t00, aCoords: t2Anim}

			pointsS = append(pointsS, []Point{p1s, p2s}...)
		}
		pointsSide = append(pointsSide, pointsS)
	}
	pointsT = append(pointsT, Point{coords: []float64{rad_a, c_height/2, 0}, n: []float32{0.0, 1.0, 0.0}, tCoords: []float32{float32(rad_a)/2, 0.0}, aCoords: []float64{rad_a, 0.0, 0.0}})
	pointsB = append(pointsB, Point{coords: []float64{rad_a, -c_height/2, 0}, n: []float32{0.0, -1.0, 0.0}, tCoords: []float32{float32(rad_a)/2, 0.0}, aCoords: []float64{rad_a, 0.0, 0.0}})
	fmt.Println(2*step1*(1 + step2))
	return &Cylinder{a: rad_a, b: rad_b, h: c_height, e_step: step1, h_step: step2, PointsT: pointsT, PointsB: pointsB, PointsSide: pointsSide}
}
...
func drawCylinder()  {
	...
	case false:
		...
		for _, p := range cyl.PointsT {
				if isText {
					gl.TexCoord2fv(&p.tCoords[0])
				}
				gl.Normal3fv(&p.n[0])
				gl.Vertex3dv(&p.coords[0])
		}
...
func key_callback(win *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
	...
	if key == glfw.KeyR && action == glfw.Press{
		//cyl = *calcCylinder(0.8, 0.8, 0.8, 30, 8)
		cyl = *calcCylinder(0.8, 0.8, 0.8, 100, 60)
		//cyl = *calcCylinder(0.8, 0.8, 0.8, 150, 80)
