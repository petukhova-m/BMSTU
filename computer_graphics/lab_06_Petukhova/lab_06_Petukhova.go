...
func calcCylinder(rad_a float64, rad_b float64, c_height float64, step1 int, step2 int) *Cylinder {
	...
	for angle := 0.0; angle < 2*Pi; angle += e_step {
		...
		tx00 := []float32{float32(x2), float32(z2)}
		tx10 := []float32{float32(x1), float32(z1)}
		tx11 := []float32{float32(x1/2), float32(z1/2)}
		tx01 := []float32{float32(x2/2), float32(z2/2)}
...
func readConfig() {
	...
	for i := 0; i < len(configLines); i++ {
		if configLines[i] != "" {
			...
			case "rX":
				rX, _ = strconv.ParseFloat(configLine[1], 64)
			case "rY":
				rY, _ = strconv.ParseFloat(configLine[1], 64)
			case "rZ":
				rZ, _ = strconv.ParseFloat(configLine[1], 64)
			case "sc":
...
func makeConfig()  {
	...
	configInfo += "rX " + strconv.FormatFloat(rX, 'E', -1, 64) + "\n"
	configInfo += "rY " + strconv.FormatFloat(rY, 'E', -1, 64) + "\n"
	configInfo += "rZ " + strconv.FormatFloat(rZ, 'E', -1, 64) + "\n"
...
func initTexture()  {
	...
	gl.TexImage2D(gl.TEXTURE_2D, 0, gl.RGBA, int32(texWidth), int32(texHeight), 0, gl.RGBA, gl.UNSIGNED_BYTE, gl.Ptr(&texData[0]))
...
func drawCylinder()  {
	...
	gl.Rotated(rX, 1.0, 0.0, 0.0)
	gl.Rotated(rY, 0.0, 1.0, 0.0)
	gl.Rotated(rZ, 0.0, 0.0, 1.0)
	...
	case true:
		...
		for _, p := range cyl.tris {
			for _, v := range p.verts {
				gl.TexCoord2fv(&v.tCoords[0])
				gl.Normal3fv(&v.n[0])
				gl.Vertex3d((1-t)*(1-t)*v.coords[0] + t*(t-1)*(v.aCoords[0]*4/3) + t*t*v.aCoords[0],
							(1-t)*(1-t)*v.coords[1] + t*(t-1)*(v.aCoords[1]+v.aCoords[1])/2 + t*t*v.aCoords[1],
							(1-t)*(1-t)*v.coords[2] + t*(t-1)*(v.aCoords[2]*4/3) + t*t*v.aCoords[2])
			}
		}
		gl.End()
	}
...
func key_callback(win *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
	...
	if key == glfw.KeyR && action == glfw.Press{
		cyl = *calcCylinder(0.7, 0.8, 0.9, 15, 8)
		sc = 1.0
		rX, rY, rZ = 0, 0, 0
		cX, cY, cZ = 0, 0, 0
		lX, lY, lZ = 0, 0, 0
